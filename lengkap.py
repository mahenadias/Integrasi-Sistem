...
tracking_collection = db["tracking"]  # Ganti dengan nama koleksi Anda
mahasiswa_collection = db["mahasiswa"]
result_collection = db["result"]
bobot_collection = db["bobot"]
...
# Fungsi untuk mengambil bobot kriteria dari koleksi "bobot" di MongoDB
def load_bobot():
    bobot = bobot_collection.find_one()  # Ambil satu dokumen dari koleksi
    if bobot:
        return bobot
    else:
...
# Fungsi untuk menyimpan bobot kriteria ke koleksi "bobot" di MongoDB
def save_bobot(bobot_kriteria):
    # Menghapus data lama (jika ada) dan menyimpan yang baru
    bobot_collection.delete_many({})  # Hapus semua data yang ada
    bobot_collection.insert_one(bobot_kriteria)  # Menyimpan bobot baru
...
# Fungsi untuk menyimpan hasil SPK ke koleksi "result" di MongoDB
def save_results(hasil_saw):
    result_collection = db["result"]  # Koleksi "result"
    tanggal = datetime.date.today().strftime("%Y-%m-%d")
    # Menyimpan hasil SPK ke dalam koleksi
    for result in hasil_saw:
        result["tanggal"] = tanggal  # Menambahkan tanggal ke setiap hasil
        result_collection.insert_one(result)  # Menyimpan setiap hasil
    st.success("Hasil SPK berhasil disimpan ke koleksi 'result' di MongoDB!")
...
def load_mahasiswa():
    mahasiswa_data = mahasiswa_collection.find()  # Mengambil semua data mahasiswa
    daftar_mahasiswa = {}
    for mahasiswa in mahasiswa_data:
        daftar_mahasiswa[mahasiswa["nim"]] = {
            "id": mahasiswa["id"],
            "nama": mahasiswa["nama"],
        }
...
# Fungsi untuk menyimpan data mahasiswa ke koleksi "mahasiswa"
def save_mahasiswa(daftar_mahasiswa):
    mahasiswa_collection.delete_many({})  # Menghapus semua data mahasiswa sebelumnya
    for nim, data in daftar_mahasiswa.items():
        mahasiswa_collection.insert_one(
            {"nim": nim, "id": data["id"], "nama": data["nama"]}
        )
...
def admin_page():
    ...
    if menu == "Dashboard":
        ...
        st.title("Live CCTV")
        cctv_url = "http://10.10.118.213:8081/"
        ...
        if nim:
            ...
            data_mahasiswa = tracking_collection.find_one(
        ...
        if st.button("Simpan Nilai"):
            ...
            # Berikan notifikasi
            st.success(f"Nilai untuk {nama_mahasiswa} berhasil disimpan!")
            save_data(
                st.session_state.data_mahasiswa,
        ...
        if not data_mahasiswa:
            ...
            if st.button("Simpan Hasil"):
                ...
                st.success("Hasil SPK berhasil disimpan")
        ...
        # Membaca daftar mahasiswa dari MongoDB
        daftar_mahasiswa = load_mahasiswa()
